...
must_haves:
  ...
  artifacts:
    - path: "package.json"
      ...
      contains: "oclif, zod dependencies"
...
Purpose: Establish the command-line interface foundation that discovers bot configurations from safe files (SOUL.md, skills, etc.) and validates data before any storage operations.
Output: Working `npx botarena generate` command with schema validation and interactive prompts.
</objective>
...
<task type="auto">
  ...
  2. Create src/cli/commands/generate.ts:
     ...
     - Scaffold file discovery for bot detection
