...
must_haves:
  truths:
    - "CLI can upload extracted bot configuration to Supabase"
  ...
  artifacts:
    - path: "src/cli/commands/upload.ts"
      provides: "CLI command for uploading profiles to Supabase"
      min_lines: 40
...
Purpose: Integrate CLI extraction with web platform by implementing profile upload to Supabase and creating dynamic profile pages for public sharing and viewing.
    ...
    async run(): Promise<void> {
      ...
      try {
        ...
        if (response.ok && result.success) {
          ...
          this.log(`🆔 Profile ID: ${result.data.id}`);
          this.log(`📅 Created: ${new Date(result.data.created_at).toLocaleDateString()}`);
...
<task type="auto">
  ...
  export async function generateStaticParams() {
    ...
    // In future, fetch from Supabase to pre-generate popular profiles
    return [];
  }
  ```
...
<task type="auto">
  ...
  import { BotProfile } from '@/lib/supabase';
...
<verification>
- CLI upload command successfully creates profiles in Supabase
